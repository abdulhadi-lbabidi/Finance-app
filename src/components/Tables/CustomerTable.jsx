...
function CustomerTable() {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((customer) =>
  ...
  const renderCell = useCallback((customer, columnKey) => {
    ...
    switch (columnKey) {
      case "name":
        return (
          <User
            avatarProps={{ radius: "full", size: "sm", src: customer.avatar }}
            classNames={{
      ...
      case "actions":
        return (
          <div className="relative flex justify-end items-center gap-2">
            <UpdateCustomerModal id={customer.id} />
