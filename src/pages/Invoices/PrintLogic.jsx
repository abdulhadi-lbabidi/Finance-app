...
import { getLogicPaysById } from "../../api";

function PrintLogic() {
  const contentRef = useRef();
  const navigate = useNavigate();
  const { type, logicId } = useParams();
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let response;
        response = await getLogicPaysById(logicId);
        setTransaction(response.data.logicPay);
      } catch (err) {
        console.error(err);
      }
    }
        ...
        <table className="w-full text-lg border-collapse">
          <tbody>
            ...
            <tr className="border-b">
              ...
              <td className="py-3">{transaction.logisticteam?.name}</td>
            </tr>
