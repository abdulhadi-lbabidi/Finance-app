...
import { AddPhoneModal, AddSocialMediaModal } from "../Modals/ReuseableModals";

export const columns = [
  { name: "ID", uid: "id", sortable: true },
  { name: "الاسم", uid: "name", sortable: true },
  { name: "نوع المدير", uid: "type", sortable: true },
  { name: "القسم او الأختصاص", uid: "department" },
  { name: "عمليات", uid: "actions" },
];

const INITIAL_VISIBLE_COLUMNS = ["name", "type", "department", "actions"];
...
function AdminTable() {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (
      ...
      filteredUsers = filteredUsers.filter((admin) =>
        Array.from(statusFilter).includes(admin.status)
      );
    }
  ...
  const renderCell = useCallback((admin, columnKey) => {
    ...
    switch (columnKey) {
      case "name":
        return (
          <User
            avatarProps={{ radius: "full", size: "sm", src: admin.avatar }}
            classNames={{
      ...
      case "type":
        return <div>{admin.type.name}</div>;
        ...
        return (
          <div className="relative flex justify-end items-center gap-2">
            ...
            <AddSocialMediaModal
              ...
              type={"admin"}
            />
            <UpdateAdminModal id={admin.id} onSaveSuccess={fetchData} />
            <DeleteAdminModal id={admin.id} onSaveSuccess={fetchData} />
          </div>
        );
      default:
        return cellValue;
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between gap-3 items-end">
          ...
          <div className="flex gap-3">
            ...
            <AddAdminModal onSaveSuccess={fetchData} />
          </div>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">
            عدد المدراء {admins.length}
          </span>
  ...
  return (
    ...
    >
      ...
      <TableBody emptyContent={"No admins found"} items={sortedItems}>
        {(item) => (
