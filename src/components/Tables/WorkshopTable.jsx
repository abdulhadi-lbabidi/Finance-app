...
import {
  ...
  DeleteWorkshopModal,
  UpdateWorkshopModal,
} from "../Modals/WorkshopModals";

export const columns = [
  { name: "ID", uid: "id", sortable: true },
  { name: "الاسم", uid: "name", sortable: true },
  { name: "العنوان", uid: "location", sortable: true },
  { name: "المالك", uid: "owner", sortable: true },
  { name: "عمليات", uid: "actions" },
];

const INITIAL_VISIBLE_COLUMNS = ["name", "location", "owner", "actions"];
...
function WorkshopTable() {
  ...
  const renderCell = useCallback((workshop, columnKey) => {
    const cellValue = workshop[columnKey];

    switch (columnKey) {
      case "owner":
        return <div>{workshop.customer.name}</div>;
      case "actions":
        return (
          <div className="relative flex justify-end items-center gap-2">
            <UpdateWorkshopModal onSaveSuccess={fetchData} id={workshop.id} />
            <DeleteWorkshopModal onSaveSuccess={fetchData} id={workshop.id} />
          </div>
        );
      default:
        return cellValue;
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between gap-3 items-end">
          ...
          <div className="flex gap-3">
            ...
            <AddWorkshopModal onSaveSuccess={fetchData} />
          </div>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">
            عدد الورشات {workshops.length}
          </span>
  ...
  return (
    ...
    >
      ...
      <TableBody emptyContent={"No workshops found"} items={sortedItems}>
        {(item) => (
