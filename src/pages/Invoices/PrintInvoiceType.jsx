...
function PrintInvoiceType() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let response;
        response = await getInvoiceById(transactionId);
        setTransaction(response.data.invoice);
      } catch (err) {
        console.error(err);
      }
    }
      ...
      >
        ...
        <table className="w-full text-lg border-collapse">
          <tbody>
            ...
            <tr className="border-b">
              <td className="py-3 font-semibold">البند:</td>
              <td className="py-3">{transaction.financeitem?.name}</td>
            </tr>
            ...
            <tr className="border-b">
              ...
              <td className="py-3">{transaction.final_price}</td>
            </tr>
...
export default PrintInvoiceType;
