...
import { getInvoiceById, getInvoiceItemsById } from "../../api";

function PrintInvoiceItem() {
  const contentRef = useRef();
  const { type, invoiceItemId } = useParams();
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let response;
        response = await getInvoiceItemsById(invoiceItemId);
        setTransaction(response.data.invoiceItem);
      } catch (err) {
        console.error(err);
      }
    }
...
export default PrintInvoiceItem;
