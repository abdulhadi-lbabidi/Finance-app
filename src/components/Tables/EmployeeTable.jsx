...
function EmployeeTable() {
  ...
  useEffect(() => {
    ...
    getEmployees()
      .then((response) => {
        setEmployees(response.data.employee); // axios puts data in response.data
        setLoading(false);
      })
      .catch((err) => {
        addToast({
  ...
  const renderCell = useCallback((employee, columnKey) => {
    ...
    switch (columnKey) {
      case "name":
        return (
          <User
            avatarProps={{ radius: "full", size: "sm", src: employee.avatar }}
            classNames={{
      ...
      case "actions":
        return (
          <div className="relative flex justify-end items-center gap-2">
            <UpdateEmployeeModal id={employee.id} />
