...
function DepartmentTable() {
  const [departments, setAdminTypes] = useState([]);
  ...
  const fetchData = () => {
    ...
    getDepartments()
      .then((response) => {
        setAdminTypes(response.data.deps); // axios puts data in response.data
        setLoading(false);
      })
      .catch((err) => {
        addToast({
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between gap-3 items-end">
          ...
          <div className="flex gap-3">
            ...
            <AddDepartmentModal onSaveSuccess={fetchData} />
          </div>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">
            عدد انواع القسمين {departments.length}
          </span>
